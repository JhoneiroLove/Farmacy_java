...
public class SalesDao {
    ...
    Connection conn;
    PreparedStatement pst; //consulta precompilada
    ResultSet rs;
    
    //Insertar - Registrar venta
    ...
    public int saleId(){ //id maximo de venta y relacionar sales con sale_details
            int id = 0;
    ...
    public List listAllSalesQuery(){
        ...
        String query = "SELECT s.id AS invoice, c.full_name AS customer, e.full_name AS employee,"
                ...
                + "employees e ON s.employee_id = e.id ORDER BY s.id ASC"; //consulta multiple
        try{
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
            rs = pst.executeQuery();
